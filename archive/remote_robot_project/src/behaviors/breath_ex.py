...
class breath_ex:
    ...
    def check_exit_flag(self):
        while not self.stop_flag:
            ...
            if data is not None:
                try:
                    # Access the data and check the value of 'data5'
                    if data['data5'] == 2:
                        self.stop_flag = True
    ...
    def run(self):
        ...
        BE_intro_thread = threading.Thread(target=self.audio_player.play_audio, args=('mp3_files/breath/breath_confirm.mp3',))
        ...
        rospy.sleep(1.0)
        intro_thread = threading.Thread(target=self.audio_player.play_audio, args=('mp3_files/breath/intro.mp3',))
        intro_thread.start()
        ...
        while not rospy.core.is_shutdown() and not self.stop_flag:
            ...
            if self.aruco_detect.breath_ex_ON:
                ...
                if time.time() > (state_start_time + state_duration):
                    ...
                    if self.state == ready and self.last_state == intro:
                        ...
                        ready_thread = threading.Thread(target=self.audio_player.play_audio, args=('mp3_files/breath/BrEx_Ready_Prompt.mp3',))
                        ready_thread.start()
                ...
                if self.state != self.last_state:
                    ...
                    if self.state == breath_in:
                        breath_in_thread = threading.Thread(target=self.audio_player.play_audio, args=('mp3_files/breath/breatheIn.mp3',))
                        self.silent_cycle_count += 1
                        breath_in_thread.start()
                    elif self.state == hold_1:
                        hold_1_thread = threading.Thread(target=self.audio_player.play_audio, args=('mp3_files/breath/hold.mp3',))
                        hold_1_thread.start()
                    elif self.state == breath_out:
                        breath_out_thread = threading.Thread(target=self.audio_player.play_audio, args=('mp3_files/breath/breatheOut.mp3',))
                        breath_out_thread.start()
                    elif self.state == hold_2:
                        hold_2_thread = threading.Thread(target=self.audio_player.play_audio, args=('mp3_files/breath/holdAgain.mp3',))
                        hold_2_thread.start()
                    elif self.state == outro:
                        outro_thread = threading.Thread(target=self.audio_player.play_audio, args=('mp3_files/breath/nice_job.mp3',))
                        outro_thread.start()
                        rospy.sleep(4.0)

                        outro_thread = threading.Thread(target=self.audio_player.play_audio, args=('mp3_files/breath/BrEx_Go_again.mp3',))
                        outro_thread.start()
                        ...
                        if self.touch_detect.head_touched:
                            ...
                            breath_out_thread= threading.Thread(target=self.audio_player.play_audio, args=('mp3_files/breath/BrEx_Timeout.mp3',))
                            breath_out_thread.start()
            ...
            #     exit_thread= threading.Thread(target=self.audio_player.play_audio, args=('mp3_files/breath/BrEx_Timeout.mp3',))
            #     exit_thread.start()
            #     exit_thread.join()
            #     # end audio stream
