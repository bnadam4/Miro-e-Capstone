...
class interactive_standby:
    def __init__(self):
        ...
        self.exit_interactive_standby=False
    ...
    def loop(self):
        ...
        while self.exit_interactive_standby == False:
            new_data = get_data()
            
            # Check if new_data is not None and has the expected keys
            if new_data is not None:
                # Check if the new data is different from the previous data
                if new_data != self.previous_data:
                    print(f"Data changed! Last data: {self.previous_data}, Current data: {new_data}")
                    ...
                    try:
                        ...
                        print(f"KeyError: Missing key {e} in new_data")
                        continue  # Skip this iteration if there's a missing key
                    
                    if user_input != INTERACTIVE_STANDBY:
                        self.exit_interactive_standby = True
                        print("Exit interactive standby")
            ...
            audiobooks_words_to_check_rumpelstiltskin = ['rumpelstiltskin', 'Crumpled', 'crumple', 'rumpole still skin']
            audiobooks_words_to_check_aladdin= ['aladdin']
            audiobooks_words_to_check_emperor= ['emperor', 'clothes']
            ...
            if 'what' in self.speech_to_text.last_text.lower() and 'do' in self.speech_to_text.last_text.lower() and 'can' in self.speech_to_text.last_text.lower() and not self.speaking:
                ...
                audio_file = 'mp3_files/interactive_standby/what_can_i_do.mp3'
            ...
            #Audio only for intro then detect by aruco and audio for audiobook selection
                ...
                # audio_file = 'mp3_files/interactive_standby/want_to_do_audiobook.mp3'
                # play_thread = threading.Thread(target=self.audio_player.play_audio, args=(audio_file,))
                # play_thread.start()
                # play_thread.join()
                # self.speech_to_text.last_text= ''
                ...
                self.stereovision.stop = True
                # Check if aduiobook detected or timeout
                start_time = time.time()
                while (time.time() - start_time) < 10:
                    self.aruco_detect.tick_camera()
                    if self.aruco_detect.rupelstiltskin or any(word in self.speech_to_text.last_text.lower() for word in audiobooks_words_to_check_rumpelstiltskin) and not self.speaking and triggered:
                        print("[AUDIOBOOK] Aruco code detected, proceeding to rupelstiltskin.")
                        self.aruco_detect.rupelstiltskin = False
                        self.speech_to_text.stop = True
                        self.book1()
                        break
                    if self.aruco_detect.emperor:
                        print("[AUDIOBOOK] Aruco code detected, proceeding to emperor.")
                        self.aruco_detect.emperor = False
                        self.speech_to_text.stop = True
                        self.book2()
                        break
                    if self.aruco_detect.aladdin or any(word in self.speech_to_text.last_text.lower() for word in audiobooks_words_to_check_aladdin) and not self.speaking and triggered:
                        print("[AUDIOBOOK] Aruco code detected, proceeding to aladdin.")
                        self.aruco_detect.aladdin = False
                        self.speech_to_text.stop = True
                        self.book3()
                        break
                    time.sleep(0.1)
                print("Timeout. No aruco detected. Exiting audibook selection.")
                break
            #Audio and aruco for Rumpelstiltskin
            elif self.aruco_detect.rupelstiltskin or any(word in self.speech_to_text.last_text.lower() for word in audiobooks_words_to_check_rumpelstiltskin) and not self.speaking and triggered:
                print("Activated the audio book: Rumpelstiltskin")
                self.aruco_detect.rupelstiltskin = False
                audiobooks_behaviour = AudiobooksBehavior()
                audiobooks_behaviour.book1_solo()
                self.aruco_detect.audiobook_ON = False
                self.stereovision.stop = True
                self.speech_to_text.stop = True
                break
            #Audio and aruco for Aladdin
            elif self.aruco_detect.aladdin or any(word in self.speech_to_text.last_text.lower() for word in audiobooks_words_to_check_aladdin) and not self.speaking and triggered:
                print("Activated the audio book: Aladdin")
                self.aruco_detect.aladdin = False
                audiobooks_behaviour = AudiobooksBehavior()
                audiobooks_behaviour.book3_solo()
                self.aruco_detect.audiobook_ON = False
                self.stereovision.stop = True
                self.speech_to_text.stop = True
                break

            #Audio and aruco for emperor new clothes
            elif self.aruco_detect.emperor or any(word in self.speech_to_text.last_text.lower() for word in audiobooks_words_to_check_emperor) and not self.speaking and triggered:
                print("Activated the audio book: emperor")
                self.aruco_detect.emperor = False
                audiobooks_behaviour = AudiobooksBehavior()
                audiobooks_behaviour.book2_solo()
                self.aruco_detect.audiobook_ON = False
                self.stereovision.stop = True
                self.speech_to_text.stop = True
                break
            ...
            if current_time - last_time >= self.delay: # every # seconds
                ...
                if self.activity_level == ACT_IDLE:
                    ...
                    """
                    audio_file = 'mp3_files/interactive_standby/hi_there.mp3'
                    play_thread = threading.Thread(target=self.audio_player.play_audio, args=(audio_file,))
                    play_thread.start()
                    """
            ...
            if not self.petted and not self.speaking:
                ...
                if self.touch_detect.head_touched:
                    ...
                    if rand_pet == 0 and last_react != 0:
                        ...
                        audio_file = 'mp3_files/interactive_standby/Petting_1.mp3'
                        play_thread = threading.Thread(target=self.audio_player.play_audio, args=(audio_file,))
                        play_thread.start()
                        ...
                        audio_file = 'mp3_files/interactive_standby/Petting_2.mp3'
                        play_thread = threading.Thread(target=self.audio_player.play_audio, args=(audio_file,))
                        play_thread.start()
                        ...
                        audio_file = 'mp3_files/interactive_standby/Petting_3.mp3'
                        play_thread = threading.Thread(target=self.audio_player.play_audio, args=(audio_file,))
                        play_thread.start()
                        # Nod MiRo's head
                    ...
                    if rand_pet == 0 and last_react != 0:
                        ...
                        audio_file = 'mp3_files/interactive_standby/Pet_back1.mp3'
                        play_thread = threading.Thread(target=self.audio_player.play_audio, args=(audio_file,))
                        play_thread.start()
                        tail_thread = threading.Thread(target=self.cosmetics_controller.wagging, args=(3.0, 15.0, ))
                        tail_thread.start()
                        ...
                        audio_file = 'mp3_files/interactive_standby/Pet_back2.mp3'
                        play_thread = threading.Thread(target=self.audio_player.play_audio, args=(audio_file,))
                        play_thread.start()
                        ...
                        audio_file = 'mp3_files/interactive_standby/Pet_back3.mp3'
                        play_thread = threading.Thread(target=self.audio_player.play_audio, args=(audio_file,))
                        play_thread.start()
                        tail_thread = threading.Thread(target=self.cosmetics_controller.wagging, args=(3.0, 15.0, ))
                        tail_thread.start()
            ...
            if not self.crinkled and not self.speaking and not self.wait and not self.petted:
                if self.speech_to_text.right_crinkle or self.speech_to_text.left_crinkle:
                    ...
                    if rand_pet == 0 and last_react != 0:
                        last_react = 0
                        audio_file = 'mp3_files/interactive_standby/Ear_touch_1.mp3'
                        play_thread = threading.Thread(target=self.audio_player.play_audio, args=(audio_file,))
                        play_thread.start()
                        ...
                        last_react = 2
                        audio_file = 'mp3_files/interactive_standby/Ear_touch_2.mp3'
                        play_thread = threading.Thread(target=self.audio_player.play_audio, args=(audio_file,))
                        play_thread.start()
                        self.move_randomly()
                    elif rand_pet == 3:
                    ...
                    audio_file = 'mp3_files/interactive_standby/Tail_touch_1.mp3'
                    play_thread = threading.Thread(target=self.audio_player.play_audio, args=(audio_file,))
                    play_thread.start()
                    head_thread = threading.Thread(target=self.joints_movement.shake, args=(1, 2, ))
                    head_thread.start()
